...
export default function NewProduct() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('products').insert({ name, slug, price: Number(price), short_description: shortDescription, stock_quantity: Number(stockQuantity), status: 'published', image_url: imageUrl, category_id: categoryId || null })
    setUploading(false)
    if (error) {
      ...
      <form onSubmit={handleSubmit} className="max-w-lg flex flex-col gap-4">
        <div>
          ...
          <input type="text" value={name} onChange={(e) => {
            ...
            setName(newName)
            if (!slugManuallyEdited) setSlug(generateSlug(newName))
          }} className="w-full bg-[#141414] border border-[#c9a24b]/20 px-3 py-2 text-white" required />
        </div>
        <div>
        ...
        <div>
          ...
          <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="w-full bg-[#141414] border border-[#c9a24b]/20 px-3 py-2 text-white">
